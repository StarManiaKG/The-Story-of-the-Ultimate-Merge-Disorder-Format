...
if(sectors.length != 2)
    UZB.die('You have to select exactly 2 sectors');
...
sectors.forEach(s => {
    ...
    if(sidedefs.length != 3)
        UZB.die(s + ' does not have exactly 3 sides');
...
if(vertices.size != 2)
    UZB.die('Expected to find 2 vertices to draw the new line between, but got ' + vertices.size);

// Merge the sectors...
UZB.Map.mergeSectors(sectors);

// ... and draw the new line
UZB.Map.drawLines(Array.from(vertices, v => v.position));